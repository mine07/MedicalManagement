...
{
    public partial class Ocupacion : System.Web.UI.Page
    {
        ...
        protected void btnAgregarOcupacion_Click(object sender, EventArgs e)
        {
            Response.Redirect("RegistroOcupacion.aspx");
        }
        ...
        protected void RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName == "Edit")
            {
                ...
                GridViewRow selectedRow = Grid_Ocupacion.Rows[index];
                ...
                System.Web.HttpContext.Current.Response.Redirect("RegistroOcupacion.aspx?Id_Ocupacion=" + selectedRow.Cells[0].Text);
            ...
            if (e.CommandName == "Delete")
            {
                ...
                int index = Convert.ToInt32(e.CommandArgument);
                GridViewRow selectedRowE = Grid_Ocupacion.Rows[index];
                try
                {
                    Response.Write("<script language=javascript>confirm('Esta seguro que quiere eliminar Ocupacion?');</script>");
                }
                catch (Exception ex)
                {
        ...
        protected void Eliminar(string id_Ocupacion)
        {
            ...
            SqlCommand command = new SqlCommand("SP_Catalogo_Ocupacion", cnn);
            command.CommandType = CommandType.StoredProcedure;
            command.Parameters.AddWithValue("@Opcion", "BAJA");
            command.Parameters.AddWithValue("@Id_Ocupacion", id_Ocupacion);
            command.ExecuteNonQuery();
            command = null;

            String Registro_Operacion_Btacora = "SP_Catalogo_Ocupacion"
                                            + "@Opcion" + " = " + "BAJA"
            ...
            comandoBitacora.Parameters.AddWithValue("@Descripcion_Bitacora", "Baja Ocupacion nueva");
        ...
        public void LlenarGridOcupacion()
        {
            ...
            if (txtBuscar_Ocupacion.Text == "")
            {
                comando.Parameters.AddWithValue("@Descripcion_Ocupacion", "");
            }
            else
            {
                comando.Parameters.AddWithValue("@Descripcion_Ocupacion", txtBuscar_Ocupacion.Text);
            }
            /*
                0  Id_Empresa
                1  Nombre_Empresa
            ...
            da.Fill(ds);
            Grid_Ocupacion.Visible = true;
            Grid_Ocupacion.DataSource = ds;
            Grid_Ocupacion.Columns[0].Visible = true;
            Grid_Ocupacion.Columns[1].Visible = true;
            Grid_Ocupacion.DataBind();
            ds.Dispose();
            da.Dispose();
