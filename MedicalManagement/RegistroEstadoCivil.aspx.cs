...
{
    public partial class RegistroEstadoCivil : System.Web.UI.Page
    {
        int Id_EdoCivil = Convert.ToInt32(System.Web.HttpContext.Current.Request.QueryString["Id_EdoCivil"]);
            ...
            else if (estatuspermiso == false)
            {
                ...
                if (!IsPostBack)
                {
                    ...
                    if (Id_EdoCivil != 0)
                    {
                        ...
                        comando.Parameters.AddWithValue("@Id_EdoCivil", Id_EdoCivil);
                        SqlDataReader reader = comando.ExecuteReader();
                        if (reader.Read())
                        {
                            Descripcion_EdoCivil.Text = reader.GetString(reader.GetOrdinal("Descripcion_EdoCivil")).Trim();
                            txtNombreCorto_EdoCivil.Text = reader.GetString(reader.GetOrdinal("NombreCorto_EdoCivil")).Trim();
                        }
        ...
        protected void btnRegresar_EdoCivil_Click(object sender, EventArgs e)
        {
            Response.Redirect("EstadoCivil.aspx");
        }

        protected void GrabaEdoCivil()
        {
            /*SqlConnection cnn = new SqlConnection(ConfigurationManager.AppSettings.Get("strConnection"));*/
            string conexion = ConfigurationManager.ConnectionStrings["ApplicationServices"].ConnectionString;
            ...
            comando.Parameters.AddWithValue("@Descripcion_EdoCivil", Descripcion_EdoCivil.Text);
            comando.Parameters.AddWithValue("@NombreCorto_EdoCivil", txtNombreCorto_EdoCivil.Text);
            ...
            if (Id_EdoCivil == 0)
            {
                Registro_Operacion_Btacora = "SP_Catalogo_EdoCivil"
                                                + "@Opcion" + " = " + "INSERTAR"
                                                + "@Descripcion_EdoCivil" + " = " + Descripcion_EdoCivil.Text;
                Descripcion_Bitacora = "Inserta Estado Civil nueva";
            }
            else
            {
                Registro_Operacion_Btacora = "SP_Catalogo_EdoCivil"
        ...
        protected void btnGuardar_EdoCivil_Click(object sender, EventArgs e) 
        {

            Alerta.InnerHtml = "";

            if (Descripcion_EdoCivil.Text.Length == 0)
            {
                Alerta.InnerHtml = "<p style=\"color: white;background-color: red\">Cuidado:Favor de Capturar la Descripción del Estado Civil</p>";
            }

            else if (txtNombreCorto_EdoCivil.Text.Length == 0)
            {
                Alerta.InnerHtml = "<p style=\"color: white;background-color: red\">Cuidado:Favor de Capturar un nombre corto para Estado Civil</p>";
            }
            else
            {
                GrabaEdoCivil();
